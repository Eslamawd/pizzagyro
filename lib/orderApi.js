...
export async function addNewOrderDelivery(orderData) {
  ...
  try {
    // لو الكود وصل هنا، معناها الـ status كانت 200/201
    return response.data;
  } catch (error) {
    // هنا بنمسك الـ 402 (Payment Required) والـ 500 وكل المشاكل
    const errorMessage =
      error.response?.data?.message ||
      error.message ||
      "An error occurred while adding delivery order";

    console.error("Order API Error:", errorMessage);

    // بنرمي الخطأ تاني عشان الـ UI (React) يقدر يمسكه ويعرض Toast
